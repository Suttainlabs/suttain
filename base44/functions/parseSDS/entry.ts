...
Deno.serve(async (req) => {
  try {
    ...
    // Use InvokeLLM with the PDF file to extract SDS data
    const result = await base44.asServiceRole.integrations.Core.InvokeLLM({
      prompt: `You are an expert chemical safety analyst. Analyze this Safety Data Sheet (SDS) PDF and extract all relevant information. 
      
      Extract and return a structured JSON with:
      1. product_name: The name of the chemical/product
      ...
      14. overall_risk_score: Number 0-100 (100 = most dangerous)
      ...
      Be thorough and accurate. If information is not available in the SDS, use null for that field.`,
      file_urls: [file_url],
