...
const FunctionDisplay = ({ toolCall }) => {
    ...
    const formattedName = name.split('.').reverse().join(' ').replace(/_/g, ' ');
    ...
    return (
        <div className="mt-2 text-xs">
            ...
            >
                ...
                <span className="text-slate-700 capitalize flex-grow">{formattedName}</span>
                {statusConfig.text && (
                ...
                {!statusConfig.spin && (toolCall.arguments_string || results) && (
                    <ChevronRight className={cn("h-3 w-3 text-slate-400 transition-transform ml-auto flex-shrink-0", 
                        expanded && "rotate-90")} />
                )}
            </button>
                        ...
                        <div>
                            ...
                            <pre className="bg-slate-50 rounded-md p-2 text-xs text-slate-600 whitespace-pre-wrap max-h-48 overflow-auto">
                                {typeof results === 'object' ? 
                                    JSON.stringify(results, null, 2) : String(results)}
                            </pre>
                        </div>
                    )}
                </div>
...
export default function MessageBubble({ message }) {
    ...
    const icon = isUser ? <User className="w-4 h-4 text-white" /> : <img src="https://qtrypzzcjebvfcihiynt.supabase.co/storage/v1/object/public/base44-prod/public/0f164d40f_Socialwhite.png" className="w-4 h-4" alt="Suttain Logo" />;
    const authorName = isUser ? "You" : "Clara";

    return (
        <div className="flex items-start gap-3 w-full">
            <div className={cn(
                "flex-shrink-0 h-8 w-8 rounded-full flex items-center justify-center mt-0.5",
                isUser ? "bg-slate-700" : "bg-slate-100 border border-slate-200"
            )}>
                {icon}
            </div>
            <div className="flex-grow min-w-0">
                <p className="font-semibold text-sm text-slate-800 mb-0.5">{authorName}</p>
                {message.content && (
                    <div className={cn(
                        ...
                        isUser ? "bg-slate-800 text-white" : "bg-white border border-slate-200"
                         ...
                         <ReactMarkdown 
                                ...
                                components={{
                                    ...
                                    a: ({ children, ...props }) => (
                                        <a {...props} target="_blank" rel="noopener noreferrer" className="text-[var(--suttain-blue)] hover:underline">{children}</a>
                                    ),
