  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border border-slate-200 shadow-sm rounded-xl p-5 space-y-4">
        <div>
          <label className="text-xs font-semibold text-slate-600 mb-1.5 block">Ingredient name</label>
          <Input
            value={chemical}
        ...
        <div className="p-3 rounded-lg bg-slate-50 border border-slate-200">
          <p className="text-[10px] uppercase tracking-widest text-slate-400 mb-1">Your Health Profile</p>
          {profileLoading ? (
        ...
        <Button onClick={handleAnalyze} disabled={loading} className="bg-[#007850] hover:bg-[#00695C] text-white">
      ...
      {result && (
        <>
          {/* Personalized header */}
          <div className="bg-gradient-to-r from-violet-50 to-teal-50 border border-violet-200 rounded-xl p-5">
            <h3 className="text-sm font-bold text-slate-900 mb-1">Personalized Risk Assessment</h3>
            <p className="text-xs text-slate-600 leading-relaxed">
              Based on your health profile as {profileSummary}, here is the protein-binding risk analysis for <span className="font-semibold text-slate-900">{result.chemical}</span>.
            </p>
          </div>
          ...
          {relevantWarnings.length > 0 && (
            <div className="bg-white border border-slate-200 shadow-sm rounded-xl p-5">
              <h3 className="text-sm font-bold text-slate-900 mb-4">Priority Warnings for Your Profile</h3>
              <div className="space-y-3">
                {relevantWarnings.map(w => {
                  ...
                  return (
                    <div key={w.key} className="flex items-start gap-3 p-3 rounded-lg bg-slate-50 border border-slate-200">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-bold text-slate-900">{labels[w.key]}</p>
          ...
          {result.safer_alternatives?.length > 0 && (
            <div className="bg-white border border-slate-200 shadow-sm rounded-xl p-5">
              <div className="flex items-center gap-2 mb-3">
                <Lightbulb className="w-4 h-4 text-[#007850]" />
                <h3 className="text-sm font-bold text-slate-900">Safer Alternatives</h3>
              </div>
              <div className="flex flex-wrap gap-2">
                {result.safer_alternatives.map((alt, i) => (
                  <span key={i} className="px-3 py-1.5 rounded-full bg-[#007850]/10 border border-[#007850]/30 text-[#007850] text-xs font-semibold">
                    {alt}
                  </span>
                ))}
              </div>
          ...
          {result.alphafold_insight && (
            <div className="bg-white border border-slate-200 border-l-4 border-l-[#007850] shadow-sm rounded-xl p-5">
              <div className="flex items-start gap-3">
                <Quote className="w-5 h-5 text-[#007850] flex-shrink-0 mt-0.5" />
                <div>
                  <p className="text-[10px] uppercase tracking-widest text-slate-400 mb-1">AlphaFold Structural Insight</p>
                  <p className="text-sm text-slate-700 leading-relaxed italic">"{result.alphafold_insight}"</p>
                </div>
              </div>
            </div>
          )}
