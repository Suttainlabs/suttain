...
import { Play, Loader2, Download, FileUp, AlertCircle } from 'lucide-react';
import Studio3DViewer from './Studio3DViewer';
import { SourcedBadge, TrustLabel, ExecutionTag } from './StudioShared';
...
export default function SingleRunPanel({ config }) {
  ...
  const handleRun = async () => {
    ...
    try {
      const res = await currentTool.handler({ input: inputValue, inputType, fileName });
      setResult(res);
      if (job) {
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = `suttain_${selectedTool}_result.json`;
    a.click();
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white border border-slate-200 rounded-2xl p-5">
        ...
        {currentTool.description && <p className="text-xs text-slate-400 mt-3">{currentTool.description}</p>}
        ...
        <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
          <div className="grid md:grid-cols-2">
            ...
            <div className="p-5">
              ...
              <button onClick={handleDownload} className="mt-4 inline-flex items-center gap-1.5 px-4 py-2 border border-slate-200 text-slate-600 rounded-lg text-sm font-semibold hover:bg-slate-50 transition-colors">
