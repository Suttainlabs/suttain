...
import { Calendar, ExternalLink, Sparkles, Archive, Plus } from 'lucide-react';
import { getToolById } from './suttainTools';

export default function ModuleCard({ module, isTeacher, onEdit, onSubmitExperiment }) {
  ...
  return (
    <Card className={`bg-white/90 border-slate-200 transition-shadow ${module.status === 'archived' ? 'opacity-60' : 'hover:shadow-md'}`}>
      <CardContent className="p-4">
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1.5">
            {isTeacher && module.status !== 'archived' && (
              <Button size="sm" variant="ghost" onClick={() => onEdit?.(module)} className="text-xs h-7 text-slate-500">
