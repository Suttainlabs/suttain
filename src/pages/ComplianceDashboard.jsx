...
import { CheckCircle2, AlertTriangle, XCircle, RefreshCw, FileText, ChevronRight } from 'lucide-react';
import { cn } from '@/lib/utils';
import { motion, AnimatePresence } from 'framer-motion';

const MARKETS = ['USA', 'EU', 'UK', 'Nigeria', 'Kenya', 'Global'];
const MARKET_KEYS = MARKETS.map(m => m.toUpperCase());
  ...
  const loadCompliance = async () => {
    ...
    try {
      const result = await base44.integrations.Core.InvokeLLM({
        prompt: `Generate a compliance status overview for a cosmetic/chemical formula in these markets: ${activeMarkets.join(', ')}.
For each market provide: status (one of: pass, review, action), affected_count (number 0-3), key_issues (array of 1-3 specific regulatory issue strings if status is not pass, empty array if pass).
Return JSON with UPPERCASE market names as keys (e.g. "USA", "EU", "UK", "NIGERIA", "KENYA", "GLOBAL").`,
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {MARKETS.map(m => {
            ...
            return (
              <button
                key={m}
                onClick={() => setActiveMarkets(prev => isActive ? prev.filter(x => x !== key) : [...prev, key])}
                className={cn('flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-semibold border-2 transition-all', isActive ? 'bg-[#02988C] text-white border-[#02988C]' : 'bg-white text-slate-600 border-slate-200 hover:border-[#02988C]/40')}
              >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
          {loading ? (
            ...
            activeMarkets.map(m => {
              ...
              const data = complianceData[m] || complianceData[displayName] || { status: 'review', affected_count: 0, key_issues: [] };
              ...
              return (
                <div key={m} className="bg-white rounded-xl border border-slate-200 p-5">
                  ...
                  {data.affected_count > 0 && (
                    <p className="text-xs text-slate-500 mb-2">{data.affected_count} affected ingredient{data.affected_count > 1 ? 's' : ''}</p>
                  )}
                  <button
                    onClick={() => setExpandedMarket(isExpanded ? null : m)}
                    className="w-full flex items-center justify-between text-xs text-[#02988C] font-semibold mt-2 hover:opacity-70 transition-opacity cursor-pointer"
                  >
                        ...
                        <div className="mt-3 pt-3 border-t border-slate-100">
                          {issues.length > 0 ? (
                            <ul className="space-y-2">
                              {issues.map((issue, i) => (
                                <li key={i} className="text-xs text-slate-600 flex items-start gap-2 bg-amber-50 rounded-lg px-3 py-2 border border-amber-100">
                  ...
                  <div className="mt-3 pt-3 border-t border-slate-100 flex gap-2">
                    <button
                      onClick={() => navigate('/SDSAnalyzer')}
                      className="flex-1 text-xs text-slate-500 hover:text-[#02988C] font-medium flex items-center gap-1 justify-center transition-colors cursor-pointer py-1.5 rounded-lg hover:bg-slate-50"
                    >
