...
import { FlaskConical, Plus, Users, BookOpen, ArrowRight } from 'lucide-react';
...
export default function StudentLab() {
  ...
  const fetchClassrooms = useCallback(async () => {
    try {
      ...
      // Filter to classrooms where the student is in the roster
      ...
      <div className="page-wrapper">
        <div className="content-container">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
            ...
            <p className="text-slate-500 text-sm">Join a classroom with a code from your teacher, then submit experiment ideas for AI evaluation.</p>
          </motion.div>
  ...
  // Classroom selected — show experiments and submit button
  return (
    <div className="page-wrapper">
      <div className="content-container">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            ...
            <Button onClick={() => setShowSubmit(true)} className="bg-violet-600 hover:bg-violet-700">
              <Plus className="w-4 h-4 mr-1.5" /> Submit Experiment
            </Button>
          </div>
        </motion.div>
        ...
        {experiments.length === 0 ? (
          <Card className="bg-white/80 border-dashed border-2 border-slate-200">
            <CardContent className="p-10 text-center">
              ...
              <p className="text-sm text-slate-500 mb-4">No experiments submitted yet. Submit your first experiment idea!</p>
              <Button onClick={() => setShowSubmit(true)} className="bg-violet-600 hover:bg-violet-700">
                <Plus className="w-4 h-4 mr-1.5" /> Submit Experiment
              </Button>
            </CardContent>
